...
import { ConvertChildToCbObjResp, ReactElasticCarouselProps } from "./Carousel.interface";
import { nextItemAction, prevItemAction } from "../actions/itemsActions";
import { activeIndexReducer } from "../reducers/items";
import { noop, pipe, numberToArray, cssPrefix } from "../utils/helpers";
...
const DEFAULT_ITEMS_TO_SHOW = 1;
const DEFAULT_ACTIVE_INDEX = 0;
const DEFAULT_ITEMS_TO_SCROLL = 1;
...
class Carousel extends Component<ReactElasticCarouselProps> {
  isComponentMounted = false;
  autoPlayIntervalId: string | number | NodeJS.Timeout | undefined | null = null;
  slider: any = null;
  ro: any = null;
  ...
  setRef = (name: any) => (ref: any) => {
    console.log('setRef', name, ref)
    if (!ref || !name) return
    this[name] = ref
  };
  ...
  initResizeObserver = () => {
    console.log('initResizeObserver', this.sliderContainer, this.slider)

    if (!this.sliderContainer || !this.slider) return
    ...
    console.log('this.sliderContainer-RO', this.ro)

    this.ro.observe(this.sliderContainer);
    this.ro.observe(this.slider);
  };

  onSliderResize = (sliderNode: any) => {
    ...
    const {
      verticalMode,
      children,
      itemsToShow: inputItemsToShow
    } = this.getDerivedPropsFromBreakPoint();

    const itemsToShow = inputItemsToShow === undefined ? DEFAULT_ITEMS_TO_SHOW : inputItemsToShow;
  ...
  getNextItemIndex = (currentIndex: number, getPrev: boolean) => {
    const {
      children,
      itemsToShow: inputItemsToShow,
      itemsToScroll
    ...
    const nextAction = getPrev
      ? prevItemAction(0, itemsToScroll || 1)
      : nextItemAction(limit, itemsToScroll || 1);
  ...
  onContainerResize = (sliderContainerNode: any) => {
    ...
    const {
      outerSpacing: inputOuterSpacing,
      verticalMode: initialVerticalMode
    } = this.getDerivedPropsFromBreakPoint();

    const outerSpacing = inputOuterSpacing || 0;
    ...
    this.setState(
      ...
      () => {
        ...
        this.setState(
          (currentState: any) => {
            console.log('any currentState', currentState)
            ...
            const maxItemsToShow = Math.min(childrenLength, itemsToShow || DEFAULT_ITEMS_TO_SHOW);
          ...
          () => {
            ...
            */
            ...
            if (typeof onResize === 'function' && typeof currentBreakPoint === 'function') onResize(currentBreakPoint);
          }
        );
      }
    );
  ...
  generatePositionUpdater = (
    direction: any, nextItemId: any, verticalMode: any, rest: any
  ) => (state: any) => {
  ...
  goTo = (nextItemId: any) => {
    ...
    const { activeIndex: inputActiveIndex } = this.state;

    const itemsToShow = inputItemsToShow === undefined ? DEFAULT_ITEMS_TO_SHOW : inputItemsToShow;

    const activeIndex: number = inputActiveIndex === undefined ? DEFAULT_ACTIVE_INDEX : inputActiveIndex;
  ...
  getNumOfPages = () => {
    const { children, itemsToShow: inputItemsToShow } = this.getDerivedPropsFromBreakPoint();
  ...
  updateActivePage = () => {
    this.setState((state: any) => {
      const { itemsToShow: inputItemsToShow, children } = this.getDerivedPropsFromBreakPoint();
  ...
  tiltMovement = (position: any, distance = 20, duration = 150) => {
    this.setState(state => {
      return {
        isSwiping: true,
  ...
  convertChildToCbObj = (index: any): ConvertChildToCbObjResp => {
    ...
    // Verificar se 'child' é uma instância de ReactElement
    if (isValidElement(child)) {
      return { item: child.props, index: roundedIdx };
    } else {
      // Lidar com o caso em que 'child' não é uma instância de ReactElement
      // Pode ser uma string ou um número, dependendo da estrutura das children
      return { item: '', index: roundedIdx };
    }
  };

  removeSliderTransitionHook = (fn: any) => {
    this.slider.removeEventListener("transitionend", fn);
  };

  getNextItemObj = (getPrev: any) => {
    console.log('getNextItemObj', getPrev)
  ...
  onNextStart = (options: any) => {
    const { onNextStart } = this.getDerivedPropsFromBreakPoint();
    const { activeIndex } = this.state;
    const nextItemObj = this.getNextItemObj(false);
    const prevItemObj = this.convertChildToCbObj(activeIndex);
    if (onNextStart) onNextStart(prevItemObj, nextItemObj);
    this.slideNext(options);
  };

  onPrevStart = (options: any) => {
    const { onPrevStart } = this.getDerivedPropsFromBreakPoint();
    const { activeIndex } = this.state;
    const nextItemObj = this.getNextItemObj(true);
    const prevItemObj = this.convertChildToCbObj(activeIndex);
    if (onPrevStart) onPrevStart(prevItemObj, nextItemObj);
    this.slidePrev(options);
  };

  slideNext = (options: any = {}) => {
    const { skipTilt } = options;
    const { enableTilt } = this.getDerivedPropsFromBreakPoint();
    const { activeIndex: inputActiveIndex, sliderPosition } = this.state;

    let activeIndex: number = inputActiveIndex === undefined ? DEFAULT_ACTIVE_INDEX : inputActiveIndex;
  ...
  onNextEnd = () => {
    ...
    const { activeIndex: inputActiveIndex, activePage } = this.state;
    ...
    if (typeof onNextEnd == 'function') onNextEnd(nextItemObj, activePage);
  };
  ...
  onPrevEnd = () => {
    ...
    this.setState({ transitioning: false });
    onChange && onChange(nextItemObj, activePage);
    if (typeof onPrevEnd == 'function') onPrevEnd(nextItemObj, activePage);
  };
  ...
  onSliderTransitionEnd = (fn: any) => {
    if (!this.slider) return
    this.slider.addEventListener("transitionend", fn);
  };

  componentDidUpdate(prevProps: ReactElasticCarouselProps, prevState: any) {
    ...
    const { activeIndex: inputActiveIndex, sliderContainerWidth } = this.state;
    ...
    if (prevChildrenLength !== currentChildrenLength) {
      const {
        itemsToShow: inputCalculatedItemsToShow
      } = this.getDerivedPropsFromBreakPoint();

      let calculatedItemsToShow: number = inputCalculatedItemsToShow === undefined ? DEFAULT_ITEMS_TO_SHOW : inputCalculatedItemsToShow;
  ...
  onSwiped = (data: any) => {
    ...
    const { childHeight, activeIndex: inputActiveIndex } = this.state;
    ...
    const {
      verticalMode,
      isRTL,
      itemsToScroll: inputItemsToScroll
    } = this.getDerivedPropsFromBreakPoint();

    const itemsToScroll = inputItemsToScroll === undefined ? DEFAULT_ITEMS_TO_SCROLL : inputItemsToScroll;
    ...
    //TODO: skipTilt is a hack to prevent tilt when swiping on edges
    //não entendi o uso de  || func({ skipTilt: true });
    func();
  };
